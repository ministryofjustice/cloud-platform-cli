...
func RdsDriftBot(cmd *cobra.Command, args []string) error {
	...
	// Load CSV either from local file or download from S3.
	...
	// For each namespace, collect all error messages.
	namespaceMap := make(map[string][]string)
	for _, record := range records {
		if len(record) < 2 {
			continue // skip if record doesn't have at least namespace and one error message
		}
		namespace := strings.TrimSpace(record[0])
		// Combine the rest of the fields into one error message.
	...
	for namespace, errorsList := range namespaceMap {
		// Combine all error messages for this namespace.
...
func processRecord(namespace, tfErr string, ghClient github.GithubIface) error {
	...
	for i, versions := range results.Versions {
		...
		versionDescription += fmt.Sprintf("- %s: %s → %s\n", moduleName, tfVersion, actualVersion)
	}
