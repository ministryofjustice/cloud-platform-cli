...
func RdsDriftChecker(cmd *cobra.Command, args []string) error {
	...
	log.Printf("Total unique namespaces found: %d\n", len(namespaceMap))
	...
	if len(successes) > 0 {
		fmt.Println("Successfully created PRs:")
	...
	if len(failures) > 0 {
		fmt.Println("\nFailed to process the following namespaces:")
	...
	if len(successes) == 0 && len(failures) == 0 {
		fmt.Println("No namespaces were processed.")
	}
...
func processRecord(namespace, tfErr string, ghClient github.GithubIface) (string, error) {
