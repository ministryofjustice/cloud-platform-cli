...
func updateVersion(moduleName, actualDbVersion, terraformDbVersion, tfDir string) ([]string, error) {
	...
	if strings.Contains(dbValue, "var.") {
		...
		updated, varFile, err := updateVersionVariable(fileName, moduleName, actualDbVersion, terraformDbVersion, tfDir, varName)
		if err != nil {
			return nil, err
		}
		if updated && varFile != "" {
...
func updateVersionHardcode(moduleName, fileName, actualDbVersion, terraformDbVersion, tfDir string) (bool, error) {
	sedCmd := fmt.Sprintf(
		"sed -i -E '/module \"%s\"/,/^}/s/db_engine_version[[:space:]]*=[[:space:]]*\"%s\"/db_engine_version = \"%s\"/' %s",
...
func updateVersionVariable(fileName, moduleName, actualDbVersion, terraformDbVersion, tfDir, varName string) (bool, string, error) {
